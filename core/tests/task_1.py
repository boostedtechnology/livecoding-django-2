...
class PetMedicalConditionTests(TestCase):
    ...
    def test_list_medical_conditions(self):
        ...
        # Act
        response = self.client.get(reverse('pet-medical-condition-list-create', kwargs={'pk': pet.id}))
    ...
    def test_list_medical_conditions_not_found(self):
        # Act
        response = self.client.get(reverse('pet-medical-condition-list-create', kwargs={'pk': uuid.uuid4()}))
    ...
    def test_create_medical_condition(self):
        ...
        # Act
        response = self.client.post(reverse('pet-medical-condition-list-create', kwargs={'pk': pet.id}), {'condition': MedicalCondition.CANCER })
    ...
    def test_create_medical_condition_not_found(self):
        # Act
        response = self.client.post(reverse('pet-medical-condition-list-create', kwargs={'pk': uuid.uuid4()}), {'condition': MedicalCondition.CANCER })
    ...
    def test_delete_medical_condition(self):
        # Arrange
        owner = Owner.objects.create(first_name="Jackson", last_name="Why", province="ON")
        pet = Pet.objects.create(name="Fido", species="DOG", age=3, owner=owner)
        medical_condition = PetMedicalCondition.objects.create(pet=pet, condition=MedicalCondition.DIABETES)
        self.destroy_url = reverse('pet-medical-condition-destroy', kwargs={'pk': pet.pk, 'condition_pk': medical_condition.pk})
    ...
    def test_delete_medical_condition_not_found(self):
        # Act
        response = self.client.delete(reverse('pet-medical-condition-destroy', kwargs={'pk': uuid.uuid4(), 'condition_pk': uuid.uuid4()}))
